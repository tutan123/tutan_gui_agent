...
export class ADB {
  ...
  private static quoteExe(exe: string): string {
    // Avoid quoting bare commands like `adb` on Windows; quoting can break resolution in some shells.
    if (!exe) return exe;
  ...
  public static async getAdbPath(): Promise<string> {
    ...
    // 1. Prefer system PATH (user's expectation: "adb ..." just works after PATH is configured)
    try {
    ...
    // 2. Check environment variable (explicit override when PATH is not available)
  ...
  static async listDevices(): Promise<ADBDeviceInfo[]> {
    try {
      ...
      const adbExe = this.quoteExe(adb);
      const { stdout } = await execAsync(`${adbExe} devices -l`);
  ...
  static async dumpHierarchy(serial: string): Promise<string> {
    ...
    try {
      ...
      await execAsync(`${adbExe} -s ${serial} shell uiautomator dump ${remotePath}`);
      
      // 2. Read file content directly using cat to avoid pull/file system issues
      const { stdout } = await execAsync(`${adbExe} -s ${serial} shell cat ${remotePath}`);
  ...
  static async tap(serial: string, x: number, y: number): Promise<void> {
    const adb = await this.getAdbPath();
    const adbExe = this.quoteExe(adb);
    await execAsync(`${adbExe} -s ${serial} shell input tap ${x} ${y}`);
  }
  ...
  static async swipe(serial: string, x1: number, y1: number, x2: number, y2: number, duration: number = 300): Promise<void> {
    const adb = await this.getAdbPath();
    const adbExe = this.quoteExe(adb);
    await execAsync(`${adbExe} -s ${serial} shell input swipe ${x1} ${y1} ${x2} ${y2} ${duration}`);
  }
  ...
  static async typeText(serial: string, text: string): Promise<void> {
    ...
    try {
      ...
      await execAsync(`${adbExe} -s ${serial} shell am broadcast -a ADB_INPUT_B64 --es msg "${b64}"`);
    } catch (e) {
      const escaped = text.replace(/\s/g, '%s');
      await execAsync(`${adbExe} -s ${serial} shell input text "${escaped}"`);
    }
  }
  ...
  static async sendKey(serial: string, key: string | number): Promise<void> {
    const adb = await this.getAdbPath();
    const adbExe = this.quoteExe(adb);
    await execAsync(`${adbExe} -s ${serial} shell input keyevent ${key}`);
  }
  ...
  static async getScreenSize(serial: string): Promise<{ width: number, height: number }> {
    const adb = await this.getAdbPath();
    const adbExe = this.quoteExe(adb);
    const { stdout } = await execAsync(`${adbExe} -s ${serial} shell wm size`);
  ...
  static async takeScreenshot(serial: string): Promise<Buffer> {
    ...
    try {
      ...
      const { stdout } = await execAsync(`${adbExe} -s ${serial} exec-out screencap -p`, {
        encoding: 'buffer',
  ...
  static async setupADBKeyboard(serial: string): Promise<void> {
    ...
    try {
      await execAsync(`${adbExe} -s ${serial} shell ime set com.android.adbkeyboard/.AdbIME`);
    } catch (e) {
